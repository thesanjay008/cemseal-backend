...
class CreateOrdersTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('orders', function (Blueprint $table) {
			...
			$table->enum('payment_status',['COD','Online'])->nullable();
            $table->integer('payment_method_id')->length(11)->nullable();
            $table->string('order_date')->nullable();
            $table->enum('status',['Temporary','New','Accepted','Preparing','Dispatched','Out-For-Delivery','On-Hold','Delivered','Canceled','Failed'])->default('Temporary');
